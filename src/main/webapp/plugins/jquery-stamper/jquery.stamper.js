...
(function($) {
	$.fn.stamper = function(options) {
		...
		this.each(function(index) {
			...
			var imgId = imgId = "jquery_stamper_img_" + (new Date().getTime()) + "_" + index;
			target.attr("data-stamper-img-id", imgId);
			...
			img.onload = function() {
				...
				var imgObj = getImageElement(imgId, _parent, img.src)
					.css({
						position : "absolute",
						//position: "relative",
						opacity : "0",
	...
	/** 获取img的DOM对象 **/
	function getImageElement(id, parent, imgsrc) {
		var img = $("#" + id);
		if (img.length == 0) {
			return $("<div class='sign'><img id='" + id + "' src='" + imgsrc + "'/></div>").appendTo(parent).hide();
		} else {
			return img.hide();
		}
	}
