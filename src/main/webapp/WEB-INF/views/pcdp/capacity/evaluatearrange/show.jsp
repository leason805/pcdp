		...
		<div class="box">
			...
			<div class="box-content">
				<form class="form-horizontal" model="form" id="mForm" method="post" action="
					<c:choose>
       					<c:when test="${empty model}">
							${ctx}/system/capacity/evaluatearrange/create.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/system/capacity/evaluatearrange/update/${model.id}.htm
       					</c:otherwise>
					</c:choose>">
					...
					<div class="form-group ">
						<label class="col-sm-2 control-label">考核名称</label>
						<div class="col-sm-6">
							<select class="populate placeholder" name="projectId" id="projects">
									<option value=""></option>
									<c:forEach items="${projects}" var="project">
										<option value="${project.id}" <c:if test="${project.id == model.project.id }">selected="selected"</c:if>>${project.name}</option>
									</c:forEach>
								</select>
						</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<select class="populate placeholder" name="invigilator" id="users">
									<option value=""></option>
						...
						<label class="col-sm-2 control-label">评分老师 </label>
						<div class="col-sm-4">
							<select class="populate placeholder" name="adjudicator" id="judgers">
									<option value=""></option>
									<c:forEach items="${users}" var="user">
										<option value="${user.id}" <c:if test="${user.id == model.adjudicator.id }">selected="selected"</c:if>>${user.name}</option>
									</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" id="startTime" placeholder="日期" data-toggle="tooltip" data-placement="bottom" title="培训日期" name="startTime" value="${model.startTime }">
						</div>
						<label class="col-sm-2 control-label">考核地点 </label>
						<div class="col-sm-4">
							...
							<div class="easyui-layout" style="width:800px;height:420px;">
								<div data-options="region:'east',split:true" title="已关联用户" style="width:380px;">
									<table id="assignedusers" class="easyui-datagrid" style="height:370px"
										data-options="rownumbers:true,singleSelect:false,url:'${ctx}/system/capacity/evaluatearrange/loaduser.htm?id=${model.id}&type=assigned',method:'get'">
										<thead>
											<tr>
								...
								<div data-options="region:'west',split:true" title="未关联用户" style="width:380px;">
									<table id="unassignedusers" class="easyui-datagrid" style="height:370px"
										data-options="rownumbers:true,singleSelect:false,url:'${ctx}/system/capacity/evaluatearrange/loaduser.htm?id=${model.id}&type=unassigned',method:'get'">
										<thead>
											<tr>
												<th data-options="field:'id',checkbox:true"></th>
												<th data-options="field:'company',width:90">所属公司</th>
