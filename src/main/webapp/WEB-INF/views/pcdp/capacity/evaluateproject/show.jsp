				...
				<form class="form-horizontal" role="form" method="post" action="
					<c:choose>
       					<c:when test="${empty model}">
							${ctx}/system/capacity/evaluateproject/create.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/system/capacity/evaluateproject/update/${model.id}.htm
       					</c:otherwise>
					</c:choose>">
					...
					<div class="form-group ">
						...
						<div class="col-sm-6">
							<select class="populate placeholder" name="categoryId" id="categories">
									<option value=""></option>
									<c:forEach items="${categories}" var="category">
										<option value="${category.id}" <c:if test="${category.id == model.category.id }">selected="selected"</c:if>>${category.name}</option>
									</c:forEach>
								</select>
						</div>
