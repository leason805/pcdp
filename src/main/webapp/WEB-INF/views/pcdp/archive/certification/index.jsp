									...
									<div class="box">
										...
										<c:choose>
											<c:when test="${not empty child.fileName }">
												<div class="dz-preview dz-processing dz-image-preview dz-success dz-complete dz-message"  style="text-align: center;">  
													<div class="dz-image">
														...
														<c:choose>
															<c:when test="${child.expired}">
																<img src="${ctx}/img/file-expired.png" class="thum_img" alt="file" data-dz-thumbnail="">
															</c:when>
															<c:otherwise>
																<c:choose>
																	<c:when test="${child.status == 'PENDING' }">
																		<img src="${ctx}/img/file-pending.png" class="thum_img" alt="file" data-dz-thumbnail="">
																	</c:when>
																	
																	<c:when test="${child.status == 'PASS' }">
																		<img src="${ctx}/img/file-pass.png" class="thum_img" alt="file" data-dz-thumbnail="">
																	</c:when>
																	<c:when test="${child.status == 'PENDING' }">
																		<img src="${ctx}/img/file-reject.png" class="thum_img" alt="file" data-dz-thumbnail="">
																	</c:when>
													...
													<div style="padding-top: 8px; ">
														...
														<a href="#" style="color: #428bca;" onclick="edit(1,${child.id}, ${child.userId}, '${child.fileName }');">查看</a>&nbsp;&nbsp;
														<c:if test="${child.status != 'EXPIRED' && !child.expired}">
															<a href="#" style="color: #428bca;" onclick="editdate(${child.id}, '${child.startDate }', '${child.endDate }');">编辑</a>&nbsp;&nbsp;
														</c:if>
														<a href="#" style="color: #428bca;" onclick="del(${child.id});">删除</a>&nbsp;&nbsp;
													<!--a href="#" onclick="edit(3,undefined);">重新上传</a-->
													</div>
												</div>
											</c:when>
											<c:otherwise>
												<form action="${ctx}/system/archive/certification/upload.htm" class="dropzone">
													<input type="hidden" name="userId" value="${userInfo.userId}" />
	  												<input type="hidden" name="typeId" value="${child.categoryId }" />		
												</form>
											</c:otherwise>
										</c:choose>
											...
											<!--div id="google-chart-6" style="min-height: 200px;" class="dropzone_" id="drop_${child.id }">
											
												<div class="dropzone_">
														
												</div>
												
												<div class="qq-dropped-zone dropzone_"><i class="fa fa-cloud"></i>点击 或 拖入文件进行上传</div>
    ...
    <div style="display:none">
    	...
    	<div class="form-group" id="bottom_">
			<div class="col-sm-offset-4 col-sm-2">
				<button type="submit" class="btn btn-primary btn-label-left" onclick="deletefile();">
		...
		<span id="date_header_">有效期</span>
    	<span id="date_inner_">
    		<div class="form-group ">
				<label class="col-sm-2 control-label">有效日期</label>
				<div class="col-sm-4">
					<input type="text" class="form-control" placeholder="起始时间如20110102" data-toggle="tooltip" data-placement="bottom" title="如20110102" name="startDate" id="startDate" value="">
				</div>
				<label class="col-sm-1 control-label">至</label>
				<div class="col-sm-4">
					<input type="text" class="form-control" placeholder="结束时间如20120102" data-toggle="tooltip" data-placement="bottom" title="如20120102" name="endDate" id="endDate" value="">
				</div>
			</div>
    	</span>
    	<div class="form-group" id="date_bottom_">
			<div class="col-sm-offset-4 col-sm-2">
				<button type="submit" class="btn btn-primary btn-label-left" onclick="updateDate();">
					<span><i class="fa fa-clock-o"></i></span>
						确定
				</button>
			</div>
...
function refresh(){
	LoadAjaxContent('${ctx}/system/archive/certification/index.htm');
}

function edit(type, id, user, fileName){
	if(type == 1){
...
function deletefile(){
	if(fid){
		var url = '${ctx}/system/archive/certification/delete/' + fid + ".htm";
		$.ajax({
...
function setdate(){
	var header = $('#date_header_').html();
	var inner = $('#date_inner_').html();
	var bottom = $("#date_bottom_").html();
...
function updateDate(){
	 ...
	 if(!DATE_FORMAT.test(startdate) || !DATE_FORMAT.test(enddate)){
		 alert('日期格式不正确，请使用格式如yyyymmdd');
		 return;
	 }
	 
	 var aj = $.ajax( {  
		url:'${ctx}/system/archive/certification/updatedate.htm',// 跳转到 action  
		data:{  
			startdate : startdate,  
			enddate : enddate,
			id: fid
...
function editdate(id, startdate, enddate){
	fid = id;
	setdate();
...
$(document).ready(function() {
	...
	$(".dropzone").dropzone({
        url: "${ctx}/system/archive/certification/upload.htm",
        dictDefaultMessage: "<div class='qq-dropped-zone' style='padding-top:20px;'><i class='fa fa-cloud'></i>点击 或 拖入文件进行上传</div>",
        //maxFiles: 10,
       // maxFilesize: 512,
        ...
        init: function() {
            this.on("success", function(file, responseText) {
            	...
            	var text = '<span style="color:red">文件已上传</span>';
            	file.previewTemplate.appendChild(Dropzone.createElement(text));
            	//document.createTextNode(text)
            	//var text = '<div style="padding-top: 8px"><a href="#" onclick="edit(1,' + id + ');">查看</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(2,' + id + ');">删除</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(3,' + id + ');">重新上传</a></div>';
            	//file.previewTemplate.appendChild(Dropzone.createElement(text));
            	//alert('文件已上传');
            ...
            this.on("addedfile", function(file) {
            	...
            	if (!file.type.match(/image.*/)) {
            	    ...
            	    this.emit("thumbnail", file, "${ctx}/img/logo-200.png");
