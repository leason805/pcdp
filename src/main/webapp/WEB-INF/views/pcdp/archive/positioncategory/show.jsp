		...
		<div class="box">
			...
			<div class="box-content">
				<form class="form-horizontal" model="form" method="post" action="
					<c:choose>
       					<c:when test="${empty model}">
							${ctx}/system/archive/positioncategory/create.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/system/archive/positioncategory/update/${model.id}.htm
       					</c:otherwise>
					</c:choose>">
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<select class="populate placeholder" name="categoryType" id="topmenus">
									<option value=""></option>
									<option value="DEPARTMENT" <c:if test="${model.categoryType == 'DEPARTMENT'}">selected="selected"</c:if>>部门</option> 
									<option value="POSITION" <c:if test="${model.categoryType == 'POSITION'}">selected="selected"</c:if>>职位</option>
									<option value="TECHLEVEL" <c:if test="${model.categoryType == 'TECHLEVEL'}">selected="selected"</c:if>>技术等级</option>
									<option value="POSITIONLEVEL" <c:if test="${model.categoryType == 'POSITIONLEVEL'}">selected="selected"</c:if>>岗位等级</option>
							</select>
						</div>
					</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="顺序" name="order" value="${model.order }">
						</div>
						<label class="col-sm-2 control-label"></label>
						<div class="col-sm-4">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label" for="form-styles">描述</label>
						<div class="col-sm-10">
