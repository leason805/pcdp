				...
				<form class="form-horizontal" model="form" method="post" action="
					<c:choose>
       					<c:when test="${empty model}">
							${ctx}/system/archive/userinfo/createbasic.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/system/archive/userinfo/updatebasic/${model.id}.htm
       					</c:otherwise>
					</c:choose>">
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="手机" data-toggle="tooltip" data-placement="bottom" title="手机" name="telephone" value="${model.telephone }">
						</div>
						...
						<div class="col-sm-4">
							<div class="radio-inline">
								<label>
									...
									<input type="radio" name="gender" value="MALE" <c:if test="${model.gender == 'MALE'}">checked</c:if>> 男
									<i class="fa fa-circle-o"></i>
								</label>
							</div>
							<div class="radio-inline">
								<label>
									<input type="radio" name="gender" value="FEMALE" <c:if test="${model.gender == 'FEMALE'}">checked</c:if>> 女
									<i class="fa fa-circle-o"></i>
								</label>
							</div>
						</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="出生日期" data-toggle="tooltip" data-placement="bottom" title="出生日期" id="birthday" name="birthday" value="<fmt:formatDate value="${model.birthday}" type="date" pattern="yyyyMMdd"/>" readonly="readonly">
						</div>
						<label class="col-sm-2 control-label">身份证</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="身份证" data-toggle="tooltip" data-placement="bottom" title="身份证" name="idCard" value="${model.idCard }">
						</div>
					</div>
					<div class="form-group ">
						<label class="col-sm-2 control-label">籍贯</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="籍贯" data-toggle="tooltip" data-placement="bottom" title="籍贯" name="nativePlace" value="${model.nativePlace }">
						</div>
						<label class="col-sm-2 control-label">政治面貌</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="政治面貌" data-toggle="tooltip" data-placement="bottom" title="政治面貌" name="political" value="${model.political }">
						</div>
					</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<select class="populate placeholder" name="deptId" id="depts">
									<option value=""></option>
									<c:forEach items="${deptList }" var="itm">
										<option value="${itm.id}" <c:if test="${model.deparment.id == itm.id}">selected="selected"</c:if>>${itm.name }</option> 
									</c:forEach>
							</select>
						</div>
						...
						<div class="col-sm-4">
							<select class="populate placeholder" name="positId" id="posits">
									...
									<c:forEach items="${positList }" var="itm">
										<option value="${itm.id}" <c:if test="${model.position.id == itm.id}">selected="selected"</c:if>>${itm.name }</option> 
									</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="form-group">
						<label class="col-sm-2 control-label" for="form-styles">通讯地址</label>
						<div class="col-sm-10">
								<textarea class="form-control" rows="5" name="address">${model.address }</textarea>
						</div>
					</div>
