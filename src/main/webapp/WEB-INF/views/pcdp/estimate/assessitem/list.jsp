...
function UserTable(){
	...
	oTable = $('#datatable-m').dataTable( {
		//"aaSorting": [[ 0, "asc" ]],
		"sAjaxSource": "${ctx}/system/estimate/assessitem/listdata.htm?projectId=${projectId}",
		"aoColumns": [
		              { "mData": "assess.user.name" },
		              { "mData": "status" },
		              { "mData": "assessScore" },
		              { "mData": "id" }
		 ],
		 ...
		 },
		 {
		        "mRender": function(data, type, row) {
		        	...
		        	var str = "";
		        	if(row.status == 'DRAFT'){
		        		str = '<a href="javascript:edit(1,' + data + ');">评估</a>';
		        	}
		        	if(row.status == 'COMPLETED'){
		        		str = '<a href="javascript:edit(2,' + data + ');">浏览</a>';
		        	}
		          return str;
		        },
		        "aTargets": [3]
...
function edit(type, id){
	var url = "";
	if(type == 1){
		url = "${ctx}/system/estimate/assessitem/show/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"90%", height:"100%"});
	}
	if(type == 2){
		url = "${ctx}/system/estimate/assessitem/review/" + id + ".htm";
		//$.colorbox({href:url, iframe:true, width:"90%", height:"100%"});
...
function changeProject(){
	...
	if(compId){
		LoadAjaxContent('${ctx}/system/estimate/assessitem/list.htm?projectId='+compId);
	}
}
