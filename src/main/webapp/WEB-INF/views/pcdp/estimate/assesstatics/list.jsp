...
function UserTable(){
	...
	oTable = $('#datatable-m').dataTable( {
		//"aaSorting": [[ 0, "asc" ]],
		"sAjaxSource": "${ctx}/system/estimate/assesstatics/listdata.htm?projectId=${projectId}",
		"aoColumns": [
		              { "mData": "user.name" },
		              { "mData": "status" },
		              { "mData": "assessScore" },
		 ...
		 },
		 {
		        "mRender": function(data, type, row) {
		        	...
		        	if(row.status == 'COMPLETED'){
		        		str = str + '<a href="#" onclick="edit(1,' + data + ');">评估明细</a>&nbsp;&nbsp;<a href="#" onclick="edit(1,' + data + ');">报告</a>';
		        	}
		          	return  str
		        },
		        "aTargets": [3]
...
function edit(type, id){
	var url = "";
	if(type == 1){
		url = "${ctx}/system/estimate/assesstatics/show/" + id + ".htm";
...
function changeProject(){
	...
	if(compId){
		LoadAjaxContent('${ctx}/system/estimate/assesstatics/list.htm?projectId='+compId);
	}
}
