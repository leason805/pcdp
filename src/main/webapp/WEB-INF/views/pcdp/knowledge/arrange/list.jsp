			...
			<div class="box-content no-padding table-responsive">
				<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-m">
					<thead>
						...
						<tr>
							<th colspan="5">
								<a class='iframe btn btn-primary btn-label-left' href="${ctx}//system/knowledge/arrange/show/.htm">
...
function UserTable(){
	...
	oTable = $('#datatable-m').dataTable( {
		//"aaSorting": [[ 0, "asc" ]],
		"sAjaxSource": "${ctx}/system/knowledge/arrange/listdata.htm",
		"aoColumns": [
		              { "mData": "paper.name" },
		              { "mData": "examDate" },
		              { "mData": "address" },
		 ...
		 },
		 {
		        "mRender": function(data, type, row) {
		          ...
		          return '<a href="#" onclick="edit(1,' + data + ');">编辑</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(0,' + data + ');">删除</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(2,' + data + ');">签到码</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(3,' + data + ');">记录</a>&nbsp;&nbsp;' + '<a href="#" onclick="edit(4,' + data + ');">邮件通知</a>&nbsp;&nbsp;';
		        },
		        "aTargets": [4]
		 }
		 ],
...
function edit(type, id){
	...
	if(type == 1){
		url = "${ctx}/system/knowledge/arrange/show/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"80%", height:"100%"});
	}
	if(type == 2){
		url = "${ctx}/system/knowledge/arrange/vcode/" + id + ".htm";
		window.open(url,'newwindow','top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');
		//$.colorbox({href:url, iframe:true, width:"80%", height:"100%"});
	}
	if(type == 3){
		url = "${ctx}/system/knowledge/arrange/record/" + id + ".htm";
		window.open(url,'newwindow','top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');
	}
	if(type == 4){
		url = "${ctx}/system/knowledge/arrange/email/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"80%", height:"100%"});
	}
...
function deleteAction(){
	if(fid){
		url = "${ctx}/system/knowledge/arrange/delete/" + fid + ".htm";
		var aj = $.ajax( {  
			url: url,// 跳转到 action   
			type:'post',  
			cache:false,  
			...
			success:function(data) {
				if(data.status == 'success'){
					LoadAjaxContent('${ctx}/system/knowledge/arrange/list.htm');
					CloseModalBox();
