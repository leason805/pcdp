...
function UserTable(){
	...
	oTable = $('#datatable-m').dataTable( {
		//"aaSorting": [[ 0, "asc" ]],
		"sAjaxSource": "${ctx}/system/performance/review/listdata.htm?projectId=${projectId}",
		"aoColumns": [
		              { "mData": "userName" },
		              { "mData": "status" },
		              { "mData": "percentage" },
		 ...
		 },
		 {
		        "mRender": function(data, type, row) {
		        	...
		        	if(row.status == 'COMPLETED'){
		        		str = '<a href="javascript:edit(2,' + data + ');">查看</a>';
		        	}
		          return str;
		        },
		        "aTargets": [3]
...
function edit(type, id){
	var url = "";
	if(type == 1){
		url = "${ctx}/system/performance/review/show/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"80%", height:"100%"});
	}
	if(type == 2){
		url = "${ctx}/system/performance/review/review/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"80%", height:"100%"});
	}
	if(type == 3){
