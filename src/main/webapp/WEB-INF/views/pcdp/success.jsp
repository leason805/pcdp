				...
				<div class="row">
					<div class="col-sm-4" style="border:0px solid #000;">
						<div class="knob-slider">
							<input class="knob" data-width="100" data-height="100" data-displayPrevious=true data-fgColor="#6AA6D6" data-skin="tron" data-thickness=".2" value="100">
						</div>
					</div>
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript('${ctx}/plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
...
function showTime(){
	if(times <= 0){
		<c:choose>
			...
			<c:otherwise>
				<c:choose>
					<c:when test="${forwindow}">
						<c:if test="${not empty message}">
							$("#timer").html('${message}');
						</c:if>
						var url = '${ctx}${url}.htm';
						window.opener.LoadAjaxContent(url);
						
						return;
					</c:when>
					<c:otherwise>
						var url = '${ctx}${url}.htm';
						parent.LoadAjaxContent(url);
						closeBoxFromWin();
						return;
					</c:otherwise>
				</c:choose>
	...
	times = times - 1;
	//var content = "页面将在 <span style='color:red'>" + times + "秒</span> 内刷新";
	//$("#timer").html(content);
