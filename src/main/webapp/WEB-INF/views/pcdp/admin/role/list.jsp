			...
			<div class="box-content no-padding table-responsive">
				<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-m">
					<thead>
						...
						<tr>
							<th colspan="3">
								<a class='iframe btn btn-primary btn-label-left' href="${ctx}/system/admin/role/show/.htm">
...
function UserTable(){
	...
	oTable = $('#datatable-m').dataTable( {
		//"aaSorting": [[ 0, "asc" ]],
		"sAjaxSource": "${ctx}/system/admin/role/listdata.htm",
		"aoColumns": [
		              { "mData": "name" },
		              { "mData": "description" },
		              { "mData": "id" }
		 ...
		 },
		 {
		        "mRender": function(data, type, row) {
		          ...
		          return '<a href="javascript:edit(1,' + data + ');">编辑</a>&nbsp;&nbsp;' + '<a href="javascript:edit(2,' + data + ');">关联用户</a>&nbsp;&nbsp;' + '<a href="javascript:edit(3,' + data + ');">关联权限</a>';
		        },
		        "aTargets": [2]
		 }
		 ],
...
function edit(type, id){
	var url = "";
	if(type == 1){
		url = "${ctx}/system/admin/role/show/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"65%", height:"100%"});
	}
	if(type == 2){
		url = "${ctx}/system/admin/role/users/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"60%", height:"100%"});
	}
	if(type == 3){
		url = "${ctx}/system/admin/role/perms/" + id + ".htm";
		$.colorbox({href:url, iframe:true, width:"65%", height:"100%"});
	}
