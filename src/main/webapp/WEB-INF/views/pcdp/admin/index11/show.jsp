		...
		<div class="box">
			...
			<div class="box-content">
				<form class="form-horizontal" role="form" method="post" action="
					<c:choose>
       					<c:when test="${empty index}">
							${ctx}/admin/index/create.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/admin/index/update/${index.id}.htm
       					</c:otherwise>
					</c:choose>">
				...
				<c:if test="${not empty index}">
					<c:choose>
       					<c:when test="${empty index.parent}">
							<div class="form-group">
								<label class="col-sm-2 control-label">一级指标编号</label>
								<div class="col-sm-4">
									<select class="populate placeholder" name="number" id="topindexs">
											<option value="-1">选择编号</option>
											<option value="1" <c:if test='${index.number == 1}'> selected="selected" </c:if>>1</option>
											<option value="2" <c:if test='${index.number == 2}'> selected="selected" </c:if>>2</option>
											<option value="3" <c:if test='${index.number == 3}'> selected="selected" </c:if>>3</option>
											<option value="4" <c:if test='${index.number == 4}'> selected="selected" </c:if>>4</option>
											<option value="5" <c:if test='${index.number == 5}'> selected="selected" </c:if>>5</option>
											<option value="6" <c:if test='${index.number == 6}'> selected="selected" </c:if>>6</option>
									</select>
								</div>
								<label class="col-sm-2 control-label"></label>
								<div class="col-sm-4">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<div class="input-group">
									  <span class="input-group-addon" style="font-weight: bold; font-style:italic; font-size: 16;">${index.psequence}.</span>
									  <input type="text" class="form-control" placeholder="指标编号" data-toggle="tooltip" data-placement="bottom" title="指标编号" name="number" value="${index.number}">
									</div>
								</div>
								<label class="col-sm-2 control-label"></label>
								<div class="col-sm-4">
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" placeholder="指标名称" data-toggle="tooltip" data-placement="bottom" title="指标名称" name="name" value="${index.name }">
						</div>
						<label class="col-sm-2 control-label"></label>
						<div class="col-sm-4">
				...
				<c:if test="${not empty index}">
					<c:if test="${not empty index.parent}">
						<div class="form-group ">
							<label class="col-sm-2 control-label">硬性指标</label>
							<div class="col-sm-4">
								<div class="toggle-switch toggle-switch-success">
									<label>
										<input type="checkbox" name="mandatory" value="YES" <c:if test="${index.mandatory == 'YES' }">checked="checked"</c:if>>
