		...
		<div class="box">
			...
			<div class="box-content">
				<form class="form-horizontal" role="form" id="mForm" method="post" action="
					<c:choose>
       					<c:when test="${empty model}">
							${ctx}/system/training/arrange/create.htm
       					</c:when>
       					<c:otherwise>
              				 ${ctx}/system/training/arrange/update/${model.id}.htm
       					</c:otherwise>
					</c:choose>">
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<select class="populate placeholder" name="courseId" id="courses">
									<option value=""></option>
									<c:forEach items="${courses}" var="course">
										<option value="${course.id}" <c:if test="${course.id == model.course.id }">selected="selected"</c:if>>${course.name}</option>
									</c:forEach>
								</select>
						</div>
						<label class="col-sm-2 control-label">培训老师</label>
						<div class="col-sm-4">
							<select class="populate placeholder" name="userId" id="coachers">
									<option value=""></option>
									<c:forEach items="${coachers}" var="coacher">
										<option value="${coacher.id}" <c:if test="${coacher.id == model.coacher.id }">selected="selected"</c:if>>${coacher.name}</option>
									</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="form-group ">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" id="courseDate" placeholder="日期" data-toggle="tooltip" data-placement="bottom" title="培训日期" name="courseDate" value="${model.courseDate }">
						</div>
						<label class="col-sm-2 control-label">培训时间</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" id="courseTime" placeholder="时间" data-toggle="tooltip" data-placement="bottom" title="培训时间" name="courseTime" value="${model.courseTime }">
						</div>
					</div>
...
$(document).ready(function() {
	...
	$('#courseDate').datepicker({dateFormat: "yy-mm-dd"});
