...
@Controller("pEvaluateItemController")
@RequestMapping("/system/performance/item")
public class EvaluateItemController extends BaseController {
	...
	public String nav(ModelMap modelMap, @PathVariable Long id){
		List<EIndicator> list = this.indicatorService.listTops4Category(id, Const.IndicatorType.PERFORMANCE);
		modelMap.addAttribute("list", list);
		return "/performance/item/nav";
	}
	...
	public String createitem(ModelMap modelMap, EIndicatorItem model, Long indicatorId){
		...
		return "redirect:/system/performance/item/content/"+indicatorId+".htm";
	}
	...
	public String updateitem(ModelMap modelMap, EIndicatorItem model, @PathVariable Long id){
		...
		return "redirect:/system/performance/item/content/"+dbRecord.getIndicator().getId()+".htm";
	}
